...
This repo does not implement the full VoIP protocol but is meant to showcase the receiving of FCM Payloads to the android device to create VoIP notifications for answer/decline options etc from different application and phone states

### Current working app states :

- [x] App is in foreground
- [x] App is in background
- [x] App is terminated

### Current working phone states:

- [x] App is in lockscreen ( without pin )
- [ ] App is in lockscreen ( with pin )
...
```json
{
  "message": {
    ...
    "data": {
      "type": "voip",
      "action": "voip",
      "caller_name": "Test User Name",
      "call_id": "Test User ID",
      "handle": "+1234567890"
    }
  }
}
...
If app displays issues with RNCallKeep exporting two functions of same type, navigate to

> node_modules>react-native-callkeep>android>build>java>io>wazo>callkeep>RNCallKeepModule.java
...
### This project also uses react-native-incoming-call from <https://github.com/bkdev98/react-native-incoming-call> , updated for current React Native version 15.0.1 , replace the following files:

> node_modules>reat-native>incomingcall>android>build.gradle
